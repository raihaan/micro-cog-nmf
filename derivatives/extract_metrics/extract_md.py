#this script loads in raw md .txt files from each subject and:
#1) concatenates each file to build a vertex X subj matrix of raw md, for both left and right hemisphers
#2) runs a md ~ intercept + scanner regression to remove scanner effects
#3) builds matrix of vertex X subj of residualized md 
#4) writes out .mat files for both raw and resid, left and right separately
#Usage python extract_md.py
...
#Load left thicknesses into matrix left_md
row = df_sorted['oxmg_id'].tolist()[0] #do first row to create matrix, concat from there
fname="../surfsamp/outputs/oxmg_" + str(row) + '/surfsamp/oxmg_' + str(row) + "_mid_left_MD_masked_smooth_clean_rsl.txt"
left_md = np.loadtxt(fname).reshape(1,n_vertex)
print(np.shape(left_md)) #1 x 40962
...
for row in df_sorted['oxmg_id'].tolist()[1:]:
    ...
    left_md = np.concatenate((left_md, x), axis=0)
print("raw left has", np.shape(left_md)[0], "subjects", np.shape(left_md)[1], "vertices")

#Repeat for right side
row = df_sorted['oxmg_id'].tolist()[0] #do first row to create matrix, concat from there
fname="../surfsamp/outputs/oxmg_" + str(row) + '/surfsamp/oxmg_' + str(row) + "_mid_right_MD_masked_smooth_clean_rsl.txt"
right_md = np.loadtxt(fname).reshape(1,n_vertex)
print(np.shape(right_md)) #1 x 40962
#load thickness file for rest of subjects and concatenate to make subjects X vertices matrix
for row in df_sorted['oxmg_id'].tolist()[1:]:
    fname="../surfsamp/outputs/oxmg_" + str(row) + '/surfsamp/oxmg_' + str(row) + "_mid_right_MD_masked_smooth_clean_rsl.txt"
    x = np.loadtxt(fname).reshape(1,n_vertex)
    right_md = np.concatenate((right_md, x), axis=0)
print("raw right has", np.shape(right_md)[0], "subjects", np.shape(right_md)[1], "vertices")

#valid ct matrices have dimensions subj x validvertices, masked midline vertices removed
n_rows = np.shape(left_md)[0] #same for right side
n_cols = np.shape(left_valid)[1] #same for right side

#take all rows (subjects), but only columns which are valid vertices
left_md_valid = left_md[:, left_valid].reshape(n_rows,n_cols)
right_md_valid = right_md[:, right_valid].reshape(n_rows,n_cols)


#write out in .mat format
#nmf will want vertex X subj, so transpose
out_matrix = np.transpose(np.asmatrix(left_md_valid))
fname = "left_md_raw.mat"
hdf5storage.savemat(fname, {'X': out_matrix}, format='7.3', options=options)
del out_matrix

out_matrix = np.transpose(np.asmatrix(right_md_valid))
fname = "right_md_raw.mat"
hdf5storage.savemat(fname, {'X': out_matrix}, format='7.3', options=options)
...
left_md_valid_resid = ox.residualize_mx(left_md_valid,scanner)
out_matrix = np.transpose(np.asmatrix(left_md_valid_resid))
fname = "left_md_resid.mat"
hdf5storage.savemat(fname, {'X': out_matrix}, format='7.3', options=options)
del out_matrix

right_md_valid_resid = ox.residualize_mx(right_md_valid,scanner)
out_matrix = np.transpose(np.asmatrix(right_md_valid_resid))
fname = "right_md_resid.mat"
hdf5storage.savemat(fname, {'X': out_matrix}, format='7.3', options=options)
del out_matrix
